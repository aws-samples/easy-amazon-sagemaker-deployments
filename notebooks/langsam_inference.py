...
import json
import base64
print("imported dino and other imports successfully")
...
class LangSAM():
    ...
    def predict_dino(self, image_pil, text_prompt, box_threshold, text_threshold):
        ...
        boxes, logits, phrases = predictd(model=self.groundingdino,
                                         image=image_trans,
                                         caption=text_prompt,
    ...
    def predict_PIL(self, image_pil, text_prompt, box_threshold=0.3, text_threshold=0.25):
    ...
    def predictme(self, image_pil, text_prompt, box_threshold=0.3, text_threshold=0.25):
        boxes, logits, phrases = self.predict_dino(image_pil, text_prompt, box_threshold, text_threshold)
        masks = torch.tensor([])
        if len(boxes) > 0:
            masks = self.predict_sam(image_pil, boxes)
...
def predict(model, payload):
    ...
    masks, boxes, phrases, logits = model.predictme(image_pil, text_prompt)
